...
var source = (() => {
  ...
  var parseMangaDetails = async ($2, mangaId) => {
    ...
    let titles = [(0, import_html_entities.decode)(title)];
    return {
      mangaId,
      mangaInfo: {
  ...
  var parseTags = (filters2) => {
    ...
    const tagSections = [
      ...
      // Tag section for order
      {
        id: "3",
        title: "order",
        tags: createTags(filters2.order.map((order) => ({ id: order.value, name: order.name })), "order")
      }
    ];
    return tagSections;
  };
  ...
  // src/Asura/AsuraInterceptor.ts
  var AsuraInterceptor = class extends import_types3.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers,
        referer: `${AS_DOMAIN}/`
      ...
      return [
        (0, import_types4.Section)("Asura Settings", [
          (0, import_types4.ToggleRow)("hq_thumb", {
            title: "HQ Thumbnails",
            value: getHQthumb(),
            onValueChange: Application.Selector(this, "hQthumbChange")
          }),
  ...
  var AsuraScans = class {
    constructor() {
      ...
      this.globalRateLimiter = new import_types5.BasicRateLimiter("ratelimiter", {
        numberOfRequests: 4,
        bufferInterval: 1,
        ignoreImages: true
      });
      this.requestManager = new AsuraInterceptor("main");
    }
    async initialise() {
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      switch (section.type) {
        case import_types5.DiscoverSectionType.featured:
          ...
          const f$ = load(Application.arrayBufferToUTF8String(featureResponse[1]));
        ...
        case import_types5.DiscoverSectionType.chapterUpdates:
          ...
          const u$ = load(Application.arrayBufferToUTF8String(updateResponse[1]));
        ...
        case import_types5.DiscoverSectionType.simpleCarousel:
          ...
          const p$ = load(Application.arrayBufferToUTF8String(popularResponse[1]));
    ...
    async getChapterDetails(chapter) {
      const request = {
        url: new URLBuilder(AS_DOMAIN).addPathComponent("series").addPathComponent(chapter.sourceManga.mangaId).addPathComponent("chapter").addPathComponent(chapter.chapterId).buildUrl(),
        method: "GET"
      };
      const [response, buffer] = await Application.scheduleRequest(request);
      const $2 = load(Application.arrayBufferToUTF8String(buffer));
      return parseChapterDetails($2, chapter.sourceManga.mangaId, chapter.chapterId);
    }
    async getGenres() {
      try {
        const request = {
        ...
        const [response, buffer] = await Application.scheduleRequest(request);
        const data2 = JSON.parse(Application.arrayBufferToUTF8String(buffer));
        return data2.genres?.values();
      } catch (error) {
